...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Redireciona com a mensagem apropriada
        if (isset($_POST['id'])) {
            header('Location: index.php?page=admin/usuarios&msg=Usuario_atualizado');
        } else {
            header('Location: index.php?page=admin/usuarios&msg=Usuario_criado');
        }
        exit;
    } catch (Exception $e) {
        $conn->rollBack();
                                        ...
                                        <script>
                                            document.addEventListener('DOMContentLoaded', function() {
                                                ...
                                                function toggleCampos() {
                                                    ...
                                                    const isPaciente = tipoSelecionado === 'paciente';
                                                    
                                                    camposPaciente.style.display = isPaciente ? 'block' : 'none';
                                                    camposMedico.style.display = tipoSelecionado === 'medico' ? 'block' : 'none';
                                                    
                                                    // Tornar os campos required apenas se for paciente
                                                    if (isPaciente) {
                                                        medicoSelect.setAttribute('required', '');
                                                        problemaSelect.setAttribute('required', '');
                                                    } else {
                                                        medicoSelect.removeAttribute('required');
                                                        problemaSelect.removeAttribute('required');
                                                        // Limpar as seleções quando não é paciente
                                                        medicoSelect.value = '';
